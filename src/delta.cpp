...
void DeltaPatch::save(const std::string& path) {
    ...
    if (!out)
        throw IOError("Failed to open patch file for writing: " + path);
...
DeltaPatch DeltaPatch::load(const std::string& path) {
    ...
    if (!in)
        throw IOError("Failed to open patch file for reading: " + path);

    DeltaPatch patch;
    patch.magic = read_le<u32>(in);
    if (patch.magic != 0x50415443)
        throw InvalidArchiveError("Invalid patch magic");
...
std::array<u8, 32> DeltaApplier::compute_archive_hash(const std::string& path) {
    std::ifstream in(path, std::ios::binary);
    if (!in)
        throw IOError("Failed to open archive for hashing: " + path);
...
void DeltaApplier::apply(const std::string& source_path, const std::string& patch_path,
