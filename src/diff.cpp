...
void ArchiveDiffer::print_summary(std::ostream& out, const std::string& source_name, const std::string& target_name) {
    ...
    out << "  Total:     " << std::setw(6)
        << (stats_.files_added + stats_.files_deleted + stats_.files_modified + stats_.files_unchanged) << "\n";
    ...
    for (const auto& diff : diffs) {
        switch (diff.type) {
            case FileDiff::ADDED:
                out << "A  " << diff.path;
                if (diff.new_size > 0)
                    out << " (" << format_bytes(diff.new_size) << ")";
                if (!diff.new_hash.empty())
                    out << " [" << diff.new_hash.substr(0, 8) << "]";
                out << "\n";
                break;
            case FileDiff::DELETED:
                out << "D  " << diff.path;
                if (diff.old_size > 0)
                    out << " (" << format_bytes(diff.old_size) << ")";
                if (!diff.old_hash.empty())
                    out << " [" << diff.old_hash.substr(0, 8) << "]";
                out << "\n";
                break;
