 ...
 */
enum class SimilarityMetric : u8 { COSINE = 0, EUCLIDEAN = 1 };

/**
 * @brief Embedding data types
 */
enum class EmbeddingType : u8 { FLOAT32 = 0, FLOAT16 = 1, INT8 = 2 };
...
public:
     ...
     * Space: O(bins)
     */
    static void sort_indices(std::vector<size_t>& indices, const std::vector<float>& similarities,
        ...
        for (size_t idx : indices) {
            ...
            int bin = std::min(histogram_bins - 1, static_cast<int>(normalized * (histogram_bins - 1)));
...
public:
     ...
     */
    explicit StreamingBulkLoader(size_t memory_cap_bytes = 512 * 1024 * 1024, int histogram_bins = 1000)
     ...
     */
    void process_binary_file(const std::string& binary_file, const std::string& filenames_file,
        ...
        if (filenames.size() != header.count) {
            throw std::invalid_argument("Filename count mismatch: " + std::to_string(filenames.size()) + " vs " +
                                        std::to_string(header.count));
        }

        // Calculate optimal batch size
        size_t batch_size = calculate_batch_size(header.dimension, static_cast<EmbeddingType>(header.type));
        ...
        while (processed < header.count) {
            ...
            std::cout << "  [" << (batch_num + 1) << "] Items " << processed << "-"
                      << (processed + current_batch_size - 1);
            std::cout.flush();

            // Load batch from disk
            auto batch_entries =
                load_batch_from_disk(binary_stream, header, processed, current_batch_size, filenames, metric);
        ...
        std::cout << "\n✓ Processed " << batch_num << " batches (" << format_number(total_items_processed_)
                  << " items)\n";
    }
     ...
     */
    const std::vector<VectorEntry>& get_sorted_entries() const { return sorted_entries_; }

    /**
     * @brief Get total items processed
     */
    size_t get_total_items() const { return total_items_processed_; }
    ...
    MemoryStats get_memory_stats() const {
        return {memory_cap_, embedding_dimension_, sorted_entries_.size(), embedding_dimension_ * sizeof(float),
                sorted_entries_.size() * (embedding_dimension_ * sizeof(float) + sizeof(VectorEntry))};
    }
...
private:
     ...
     */
    size_t calculate_batch_size(size_t dimension, EmbeddingType type) {
        (void)type;
     ...
     */
    std::vector<VectorEntry> load_batch_from_disk(std::ifstream& stream, const Header& header, size_t start_index,
                                                  size_t batch_size, const std::vector<std::string>& filenames,
        ...
        if (reference_embedding_.empty()) {
            auto first_emb = EmbeddingConverter::to_float(raw_bytes.data(), header.dimension, type);
        ...
        for (size_t i = 0; i < batch_size; ++i) {
            ...
            auto embedding = EmbeddingConverter::to_float(&raw_bytes[offset], header.dimension, type);
     ...
     */
    float compute_similarity(const std::vector<float>& embedding, SimilarityMetric metric) {
        if (embedding.empty())
            return 0.0f;
    ...
    void add_sorted_batch(std::vector<VectorEntry>& batch) {
        sorted_entries_.insert(sorted_entries_.end(), std::make_move_iterator(batch.begin()),
